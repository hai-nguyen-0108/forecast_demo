...
class PlotBand {
  ...
  const PlotBand(
      ...
      this.gradient,
      required this.imageBonusOffset})
      : textStyle = textStyle ??
            const TextStyle(
                fontFamily: 'Roboto',
...
class _PlotBandPainter extends CustomPainter {
  ...
  void _drawPlotBand(PlotBand plotBand, Rect plotBandRect, int textAngle,
    ...
    if (plotBandRect != null && plotBand.color != null) {
      ...
      canvas.drawImage(plotBand.backgroundImage,
          Offset(plotBandRect.left, plotBandRect.top), paint);
    }
    if (plotBand.image != null) {
      final Size textSize = measureText('', plotBand.textStyle, textAngle);
      num? x = 0;
      num? y = 0;
              ...
              (plotBandRect.left +
                      ...
                      ((plotBandRect.left ==
                          ...
                          : (plotBandRect.right ==
                              ...
                              ? textSize.width
                              : textSize.width / 2 +
                                  plotBand.imageBonusOffset) +
                      x!) +
                  (textAngle != 0 ? textSize.width / 2 : 0),
              (plotBandRect.top +
                      plotBandRect.height / 3 -
                      ((plotBandRect.bottom ==
                              stateProperties.chartAxis.axisClipRect.top)
                          ? textSize.height
                          : (plotBandRect.top ==
